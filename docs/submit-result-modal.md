...
Submitting from the task editor (`xl+` only) stays on the task page. A modal streams that submission’s eval over `/subm-updates`, filtered by `subm_uuid` (the SSE feed is global).
