  ...
  export default function SubmissionTableServer({
    ...
    return (
      <div className="overflow-x-auto">
        ...
        >
          <TableHeader>
            <TableColumn key="createdAt" width={200}>Datums & laiks</TableColumn>
              ...
              : sortedSubmissions.map((item) => (
                  ...
                  >
                    {["createdAt", "author", "task", "language", "result", "status"].map((columnKey) => (
                      <TableCell key={columnKey} className={cn("h-12 border-b-small")}>
  ...
  function renderCell(row: BriefSubmission, columnKey: string) {
    switch (columnKey) {
      case "createdAt":
        return (
          <div className="flex flex-wrap gap-x-2 gap-y-1 min-w-20">
            ...
            <span>{new Date(row.created_at).toLocaleString("lv").split(" ")[1]}</span>
          </div>
        );
      case "author":
        return row.username;
      ...
      case "status":
        return (
          <span>
            {statusTranslations[row.eval_status as keyof typeof statusTranslations]}
          </span>
        );
      default:
        return <></>;
